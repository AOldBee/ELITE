...
class FlowRequest:
    def __init__(self, source_id, des_id, node_id, seq, type):
...
# 路由回复 属性：区域路径
class FlowReply:
    def __init__(self, area_path, trans_type, L):
...
# 路由汇报，通知控制器更新路由表
class FlowReport:
    def __init__(self, area_path, trans_type, L, type, delay, hops, c_message, veh_path, loss, loss_area):
        self.area_path = area_path
        self.trans_type = trans_type
        self.loss = loss
...
class DataPkt:
    def __init__(self, source_id, des_id, pkt_size, state, node_id, seq, s_time):
        ...
        self.delay = 0 # 时延
        self.count = 0
        self.type = random.choice(['Security', 'Efficiency', 'Information', 'Entertainment'])
        self.trans_type = 0

    def insert_info(self, area_path, trans_type, L):
        self.area_path = area_path
        self.trans_type = trans_type
        self.L = L
