...
class Node:
    ...
    def generate_request(self, des_id, controller, size):
        ...
        pkt_ = Pkt.DataPkt(self.node_id, des_id, size, 0, self.node_id, self.pkt_seq, 1000 * time.time())
        self.data_pkt_list.append(pkt_)
        # 向控制器的路由请求列表中插入路由请求包 （源节点，目的节点，发出者节点，发出者序号）
        controller.flow_request_list.append(Pkt.FlowRequest(self.node_id, des_id, self.node_id, self.pkt_seq, pkt_.type))
        return
    ...
    def receive_flow(self, flow_reply):
        for pkt in self.data_pkt_list[::-1]:
            ...
            if pkt.source_id == self.node_id: #and pkt.seq == self.pkt_seq:
                pkt.insert_info(flow_reply.area_path, flow_reply.trans_type, flow_reply.L)
                break
        return
    ...
    def forward_pkt(self, node_list, controller):
        ...
        for pkt in self.data_pkt_list[::-1]:
            ...
            if next_hop != -1:
                ...
                Gp.fail_time += 1
                # 丢包
                #controller.flow_report_list.append(Pkt.FlowReport(pkt.area_path, pkt.trans_type, pkt.L, pkt.type, 0, 0, 0, loss=1, loss_area=curr_area))
    ...
    def receive_pkt(self, data_pkt, node_list, controller):
        ...
        if data_pkt.des_id == self.node_id:
            ...
            # 向控制器回复
            controller.flow_report_list.append(Pkt.FlowReport(data_pkt.area_path, data_pkt.trans_type, data_pkt.L, data_pkt.type, data_pkt.delay, hops, c_message, data_pkt.path, loss = 0, loss_area=0))
        # 当前为中间节点
        else:
